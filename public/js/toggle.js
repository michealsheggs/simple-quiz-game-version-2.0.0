  ...
  const isloggedIn = window.sessionStorage.getItem('email');
    ...
    //if session storage email 
    // $_getJSON(url:url: `http://localhost:3000/users?email=${email},){
    // $_each() loop through to check if user  indatabase email match the session storage email
    // }
    // if it matchMedia, hide the sumbit and show answer nav link
